...
void alarm_settings_changed_type(GtkToggleButton* toggle, gpointer data);

void alarm_settings_changed_label(GtkEditable* editable, gpointer data);

void alarm_settings_changed_time(GtkSpinButton* spinbutton, gpointer data);

void alarm_settings_changed_repeat(GtkToggleButton* togglebutton, gpointer data);

void set_repeat_toggle_buttons(AlarmSettingsDialog* dialog, AlarmRepeat repeat);

void alarm_settings_repeat_all(GtkButton* button, gpointer data);

void alarm_settings_repeat_weekday(GtkButton* button, gpointer data);

void alarm_settings_repeat_weekend(GtkButton* button, gpointer data);

void alarm_settings_repeat_clear(GtkButton* button, gpointer data);

void alarm_settings_changed_notify_type(GtkToggleButton* togglebutton, gpointer data);

void alarm_settings_changed_sound(GtkComboBox* combo, gpointer data);

void alarm_settings_changed_sound_repeat(GtkToggleButton* togglebutton, gpointer data);

void alarm_settings_changed_app(GtkComboBox* combo, gpointer data);
...
static void alarm_settings_update_type(AlarmSettingsDialog* dialog)
{
    ...
    if((type == ALARM_TYPE_CLOCK && clock_toggled) || (type == ALARM_TYPE_TIMER && timer_toggled)) {
        // No change
        return;
    }
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(dialog->clock_toggle), type == ALARM_TYPE_CLOCK);
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(dialog->timer_toggle), type == ALARM_TYPE_TIMER);
...
static void alarm_settings_update_label(AlarmSettingsDialog* dialog)
{
...
static void alarm_settings_update_notify_type(AlarmSettingsDialog* dialog)
{
    ...
    if((type == ALARM_NOTIFY_SOUND && sound_active && sound_sensitive && !app_sensitive) || (type == ALARM_NOTIFY_COMMAND && app_active && !sound_sensitive && app_sensitive)) {
        // No change
        return;
    }
...
static void alarm_settings_update_sound(AlarmSettingsDialog* dialog)
{
    AlarmListEntry* item;
    GList* l;
    gint pos;
...
static void alarm_settings_update_sound_repeat(AlarmSettingsDialog* dialog)
{
...
static void alarm_settings_update_app_command(AlarmSettingsDialog* dialog)
{
...
static void alarm_settings_update(AlarmSettingsDialog* dialog)
{
...
static void alarm_changed(GObject* object, GParamSpec* param, gpointer data)
{
...
static void open_sound_file_chooser(AlarmSettingsDialog* dialog)
{
    GtkWidget* chooser;

    chooser = gtk_file_chooser_dialog_new(_("Select sound file..."), GTK_WINDOW(dialog->dialog), GTK_FILE_CHOOSER_ACTION_OPEN, _("_Cancel"), GTK_RESPONSE_CANCEL, _("_Open"), GTK_RESPONSE_ACCEPT, NULL);
...
void alarm_settings_changed_type(GtkToggleButton* toggle, gpointer data)
{
    ...
    g_debug("alarm_settings_changed_type(%s) -> %s", gtk_buildable_get_name(GTK_BUILDABLE(toggle)), toggled ? "TRUE" : "FALSE");
...
static void preview_player_state_cb(MediaPlayer* player, MediaPlayerState state, gpointer data)
{
    AlarmSettingsDialog* dialog = (AlarmSettingsDialog*)data;
    const gchar* stock;
...
void alarm_settings_sound_preview(GtkButton* button, gpointer data)
{
    ...
    if(dialog->player && dialog->player->state == MEDIA_PLAYER_PLAYING) {
        ...
        if(dialog->player == NULL) {
            dialog->player = media_player_new(dialog->alarm->sound_file, dialog->alarm->sound_loop, preview_player_state_cb, dialog, media_player_error_cb,
                                              dialog->dialog);
            if(dialog->player == NULL) {
                // Unable to create player
                alarm_error_trigger(dialog->alarm, ALARM_ERROR_PLAY, _("Could not create player! Please check your sound settings."));
                return;
            }
        }
 ...
 */
static void alarm_settings_dialog_clear(AlarmSettingsDialog* dialog)
{
    if(dialog->player) {
